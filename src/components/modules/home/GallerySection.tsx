...
export function GallerySection() {
    ...
    useEffect(() => {
        galleryService.getGalleries().then(({ data }) => {
            if (data) {
                // Filter only published items
                setImages(data.filter(item => item.isPublish));
            }
            setIsLoading(false);
        });
    }, []);
    ...
    return (
        <section
            ...
            {/* Gallery Focus Crosshair Background */}
            <div className="absolute inset-0 z-0 pointer-events-none" style={{
                maskImage: "linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)",
                WebkitMaskImage: "linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)"
            }}>
                <div className="absolute inset-0 dark:opacity-[0.15] opacity-[0.15]" style={{
            ...
            <div className="container-custom mx-auto px-4 sm:px-6 lg:px-8">
                <SectionTitle
                    subtitle="Portfolio Design Gallery"
                    title="Design Gallery"
                    description="A showcase of my recent creative works and design explorations"
                />
