...
import { AboutManager } from "@/components/modules/dashboard/admin/AboutManager";
...
export default function AdminAboutPage() {
    ...
    const fetchAbout = useCallback(async () => {
        setIsLoading(true);
        const { data, error } = await aboutService.getAbouts();

        if (error) {
            toast.error("Failed to load about data", { description: error.message });
            setAbout(null);
        } else {
            // Only the first record is used (singleton pattern)
        ...
        <div className="space-y-6 min-h-screen">
            <div>
                ...
                <p className="text-muted-foreground mt-2">
                    Manage your hero and about me images displayed on the portfolio
                </p>
            </div>
